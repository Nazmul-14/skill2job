...
FILE = os.path.join("data", "cv.json")
...
class CV(tk.Frame):
    ...
    def draw(self, parent, title, key):
        ...
        for i, item in enumerate(self.data.get(key, [])):
            ...
            lbl.pack(anchor="w")
            lbl.bind("<Button-1>", lambda e, k=key, i=i, v=item: self.select_list(k, i, v))
    ...
    def export_pdf(self):
        ...
        header_text = Paragraph(
            ...
            ParagraphStyle(name='h', fontSize=18, textColor=colors.white)
        )
...
if __name__ == "__main__":
    ...
    CV(root, user_id=None).pack(fill="both", expand=True)
    root.mainloop()